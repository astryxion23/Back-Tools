...
public class BackToolLayer extends RenderLayer<PlayerRenderState, PlayerModel>
{
    ...
    @Override
    public void render(PoseStack poseStack, MultiBufferSource buffer, int packedLight, PlayerRenderState state, float yRot, float xRot)
    {
        ...
        if (!EventHandler.heldTools.containsKey(player))
        {
            return;
        }
        ...
        if ((!capeWouldHideBackTools || allowRenderDespiteCape) && !state.isInvisible && state.pose != Pose.SLEEPING)
        {
            EventHandler.HeldInfo info = EventHandler.heldTools.get(player);
            boolean enableEasterEgg = BackToolsConfig.easterEgg() && (state.pose == Pose.SWIMMING || state.isFallFlying || (state.name != null && state.name.equalsIgnoreCase("iChun")));
            ...
            renderBackItems(poseStack, buffer, packedLight, level, info.lastMain, info.lastOff, mainIsRight, tickAnim, partialTick, offset);
